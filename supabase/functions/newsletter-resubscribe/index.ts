...
// Stateless guard: block non-POST and non-JSON requests before req.json()
  ...
  try {
    const { token, email, turnstileToken }: ResubscribeRequest = await req.json();
    ...
    if (!turnstileResult.success) {
      console.warn(`Turnstile verification failed for newsletter-resubscribe: ${turnstileResult.error}`);
